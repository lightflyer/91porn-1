...
namespace M3U8_Downloader
{
    public partial class Form1 : Form
    {
        ...
        string m_proxy;
        string m3u8path;
        string history = Environment.CurrentDirectory+ "\\settings\\history";
        ...
        private void textBox_Info_TextChanged()
        {
            ...
            label1.Text = "标题:"+m_urlList[m_count].Split(",".ToCharArray())[1] +"  "+"视频信息:"+ resolution.Match(m_outPut).Value.Replace(", ", "") + "," + fps.Match(m_outPut).Value.Replace(", ", "");
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!File.Exists(Environment.CurrentDirectory + "\\settings\\histoy"))
            {
                File.Create(Environment.CurrentDirectory + "\\settings\\histoy");
            }
            ...
            {
                m_path = System.Environment.CurrentDirectory+"\\videos";
                m_proxy = "";
                CreateSettingFile(System.Environment.CurrentDirectory + "\\settings\\M3u8_Downloader_Settings.xml");
            }
...
namespace M3U8_Downloader
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Download()
        {
            ...
            m_urlList = File.ReadAllLines(m3u8path);
            m_count = 0;

            string command = "";
            if (File.ReadAllText(history).IndexOf(m_urlList[m_count].Split(",".ToCharArray())[1]) < 0)
            {
                StreamWriter sw = File.AppendText(history);
                sw.WriteLine(m_urlList[m_count].Split(",".ToCharArray())[1]);
                sw.Dispose();
                command = "-rw_timeout 10000000 -i " + "\"" + m_urlList[0].Split(",".ToCharArray())[0] + "\"" + " -c copy -y -bsf:a aac_adtstoasc -movflags +faststart " + "\"" + m_path + "\\" + m_urlList[m_count].Split(",".ToCharArray())[1] + ".mp4" + "\"";
            }
        ...
        private void CmdProcess_Exited(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (File.ReadAllText(history).IndexOf(m_urlList[m_count].Split(",".ToCharArray())[1]) < 0)
                {
                    ...
                    sw.WriteLine(m_urlList[m_count].Split(",".ToCharArray())[1]);
                    sw.Dispose();
                    command = "-rw_timeout 10000000 -i " + "\"" + m_urlList[m_count].Split(",".ToCharArray())[0] + "\"" + " -c copy -y -bsf:a aac_adtstoasc -movflags +faststart " + "\"" + m_path + "\\" + m_urlList[m_count].Split(",".ToCharArray())[1] + ".mp4" + "\"";
                }
                    RealAction(@"Tools\ffmpeg.exe", command);
                    m_count++;
            }
